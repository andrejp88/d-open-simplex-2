...
public final class OpenSimplex2F : OpenSimplex2 {
    ...
    private enum double N2 = 0.01001634121365712;
    private enum double N3 = 0.030485933181293584;
    private enum double N4 = 0.009202377986303158;
...
unittest
{
    ...
    foreach (size_t i, ref ubyte pixel; pixels)
    {
        ...
        pixel = ((openSimplex2f.noise2(x.to!double / SCALE, y.to!double / SCALE) + 1) * 127).to!ubyte;
    }


    IFImage img = IFImage(WIDTH, HEIGHT, ColFmt.Y, pixels);
    write_png("./test-results/OpenSimplex2F 2D.png", WIDTH, HEIGHT, img.pixels);
}

@("OpenSimplex2F 3D slice")
unittest
{
    import imageformats : IFImage, ColFmt, write_png;
    import std.conv : to;
    ...
    foreach (size_t i, ref ubyte pixel; pixels)
    {
        immutable size_t x = i % WIDTH;
        immutable size_t y = i / HEIGHT;
        pixel = ((openSimplex2f.noise3_XYBeforeZ(x.to!double / SCALE, y.to!double / SCALE, 0) + 1) * 127).to!ubyte;
    }


    IFImage img = IFImage(WIDTH, HEIGHT, ColFmt.Y, pixels);
    write_png("./test-results/OpenSimplex2F 3D XYbeforeZ.png", WIDTH, HEIGHT, img.pixels);
}
...
unittest
{
    ...
    {
        openSimplex2f.noise2(i.to!double, (999_999 - i).to!double);
    }
