...
public final class OpenSimplex2S : OpenSimplex2 {
    ...
    private double noise2_Base(double xs, double ys) {
        ...
        // Index to point list
        int a = cast(int)(xsi + ysi);
        int index =
            (a << 2) |
            cast(int)(xsi - ysi / 2 + 1 - a / 2.0) << 3 |
            cast(int)(ysi - xsi / 2 + 1 - a / 2.0) << 4;
    ...
    private double noise4_Base(double xs, double ys, double zs, double ws) {
        ...
        // Point contributions
        foreach (LatticePoint4D c; LOOKUP_4D[index]) {
    ...
    static this() {
        ...
        for (int i = 0; i < 256; i++) {
            LOOKUP_4D ~= new LatticePoint4D[lookup4DPregen[i].length];
    ...
    private enum double N2 = 0.05481866495625118;
    private enum double N3 = 0.2781926117527186;
    private enum double N4 = 0.11127401889945551;
...
unittest
{
    ...
    foreach (size_t i, ref ubyte pixel; pixels)
    {
        ...
        pixel = ((openSimplex2s.noise2(x.to!double / SCALE, y.to!double / SCALE) + 1) * 127).to!ubyte;
    }
...
unittest
{
    ...
    foreach (size_t i, ref ubyte pixel; pixels)
    {
        immutable size_t x = i % WIDTH;
        immutable size_t y = i / HEIGHT;
        pixel = ((openSimplex2s.noise4_Classic(x.to!double / SCALE, y.to!double / SCALE, 0, 0) + 1) * 127).to!ubyte;
    }
